...
def test_set_site_catchment_success(cursor):
    ...
    cursor.execute('INSERT INTO geo.sites (site_name, location) VALUES (%s, ST_MakePoint(%s, %s)) returning site_id', ['test site for catchment', -120, 12])
    site_id = cursor.fetchone()[0]

    # Valid polygon in Oregon
    cursor.execute("""SELECT sample.fn_set_site_catchment(%s, '{ "type": "Polygon", "coordinates": [ [ [ -122.68432617187497, 42.77927536024188 ], [ -116.53198242187499, 42.77927536024188 ], [ -116.53198242187499, 44.308126684886126 ], [ -122.68432617187497, 44.308126684886126 ], [ -122.68432617187497, 42.77927536024188 ] ] ] }')""", [site_id])
    assert cursor.fetchone()[0] == 1

    # Valid polygon spanning Oregon coast and Pacific Ocean
    cursor.execute("""SELECT sample.fn_set_site_catchment(%s, '{ "type": "Polygon", "coordinates": [ [ [ -127.28759765624999, 41.21172151054787 ], [ -121.13525390625, 41.21172151054787 ], [ -121.13525390625, 42.779275360241904 ], [ -127.28759765624999, 42.779275360241904 ], [ -127.28759765624999, 41.21172151054787 ] ] ] }')""", [site_id])
    assert cursor.fetchone()[0] == 1

    # Valid polygon spanning Canadian and US border
    cursor.execute("""SELECT sample.fn_set_site_catchment(%s, '{ "type": "Polygon", "coordinates": [ [ [ -118.37768554687497, 48.312427904071754 ], [ -112.22534179687499, 48.312427904071754 ], [ -112.22534179687499, 49.696061819115634 ], [ -118.37768554687497, 49.696061819115634 ], [ -118.37768554687497, 48.312427904071754 ] ] ] }')""", [site_id])
    assert cursor.fetchone()[0] == 1
...
def test_set_site_catchment_failure(cursor):
    ...
    assert "ERROR" in str(exinfo.value.pgerror)
