...
CREATE INDEX ix_sites_location ON geo.sites USING GIST(location);
CREATE INDEX ix_sites_catchment ON geo.sites USING GIST(catchment);
...
CREATE TABLE geo.indicator_types (
    indicator_type_id   INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE entity.lab_types (
    lab_type_id         INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE taxa.translations (
    translation_id      INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE taxa.taxa_translations (
    ...
    taxonomy_id             INT NOT NULL,
    translation_taxonomy_id INT NOT NULL,
...
CREATE INDEX projects_contact_id ON sample.projects(contact_id);
...
--     report_out          TIMESTAMPTZ, -- drop this column. use completed_date instead.
    ...
    description         TEXT,
    metadata            JSON,
    project_id          INT, -- changing this ID through the UI will need to change the sample_projects junction table
...
CREATE TABLE sample.billing (
    billing_id          INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    box_id              INT NOT NULL,
    ...
    billing_customer_id INT, -- goes into the billing tables
...
CREATE TABLE sample.mass_methods (
    mass_method_id      INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE sample.mass_types (
    mass_type_id      INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE sample.vegetation (
    ...
    CONSTRAINT fk_veg_samples_species_id FOREIGN KEY (taxonomy_id) REFERENCES taxa.taxonomy(taxonomy_id)
);
...
CREATE INDEX ix_sites_location ON sample.sample_indicators USING GIST(location);
CREATE INDEX ix_sites_catchment ON sample.sample_indicators USING GIST(catchment);
...
CREATE TABLE metric.metric_types (
    metric_type_id      INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE metric.analysis_types (
    analysis_type_id    INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
