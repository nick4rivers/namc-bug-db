...
(
    ...
    CONSTRAINT fk_metric_runs_sample_id FOREIGN KEY (sample_id) REFERENCES sample.samples (sample_id)
);
CREATE UNIQUE INDEX ux_metric_runs_sample_id ON metric.runs (sample_id, code_version);

CREATE TABLE metric.sample_metrics
(
    run_id       INT NOT NULL,
    sample_id    INT NOT NULL,
...
CREATE INDEX fx_metric_sample_metrics_sample_id ON metric.sample_metrics (sample_id);
CREATE INDEX fx_metric_sample_metrics_metric_id ON metric.sample_metrics (metric_id);

CREATE TABLE metric.box_reports
(
    box_report_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    box_id        INT         NOT NULL,
    report_id     SMALLINT    NOT NULL,
    ...
    CONSTRAINT fk_box_reports_box_id FOREIGN KEY (box_id) REFERENCES sample.boxes (box_id) ON DELETE CASCADE,
    CONSTRAINT fk_box_reports_report_id FOREIGN KEY (report_id) REFERENCES metric.reports (report_id)
);
CREATE INDEX fx_metric_box_reports_box_id ON metric.box_reports (box_id);
CREATE INDEX fx_metric_box_reports_report_id ON metric.box_reports (report_id);

CREATE TABLE metric.box_report_runs
(
    box_report_id INT NOT NULL,
    run_id        INT NOT NULL,
