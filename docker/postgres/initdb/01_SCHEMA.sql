...
CREATE TRIGGER tr_indicator_types_update BEFORE UPDATE ON geo.indicator_types FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
...
CREATE TABLE geo.indicators (
    ...
    indicator_type_id   SMALLINT NOT NULL,
    description         TEXT,
    metadata            JSON,
    created_date        TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_date        TIMESTAMPTZ NOT NULL DEFAULT now(),
    ...
    CONSTRAINT fk_indicators_indicator_type_id FOREIGN KEY (indicator_type_id) REFERENCES geo.indicator_types(indicator_type_id)
);
CREATE INDEX fx_indicators_unit_id ON geo.indicators(unit_id);
CREATE INDEX fk_indicators_indicator_type_id ON geo.indicators(indicator_type_id);
CREATE TRIGGER tr_indicators_update BEFORE UPDATE ON geo.indicators FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
...
CREATE TRIGGER tr_site_indicators_update BEFORE UPDATE ON geo.site_indicators FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
...
CREATE TRIGGER tr_organic_mater_update BEFORE UPDATE ON sample.organic_matter FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
...
CREATE TRIGGER tr_stomachs_update BEFORE UPDATE ON sample.stomachs FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
...
CREATE TRIGGER tr_sample_indicators_update BEFORE UPDATE ON sample.sample_indicators FOR EACH ROW EXECUTE PROCEDURE fn_before_update();
