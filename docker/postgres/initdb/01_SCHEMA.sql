...
(
    ...
    CONSTRAINT country_name CHECK (length(country_name) > 0),
    CONSTRAINT abbreviation CHECK (length(abbreviation) > 0)
);
...
COMMENT ON TABLE geo.countries IS 'Table containing one record for each country in the World.'
    'The Geography column is used to store a multi-polygon of the outline of the country for use '
    ' when validating sample locations.';
...
(
    ...
    CONSTRAINT ck_state_name CHECK (length(state_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0),
...
CREATE TRIGGER tr_states_update
    ...
    FOR EACH ROW
EXECUTE PROCEDURE fn_before_update();
COMMENT ON table geo.states IS 'table containing the 50 states of America. The Geography'
    ' column contains a multi-polygon of the outline of each state. These polygons are copied'
    ' or unioned together and stored in geo.models when a model pertains to a single or collection'
    ' of US states. Note that this table does NOT currently store any sub-national states for countries'
...
(
    ...
    CONSTRAINT ck_county_name CHECK (length(county_name) > 0),
...
CREATE TRIGGER tr_counties_update
    ...
    FOR EACH ROW
EXECUTE PROCEDURE fn_before_update();
COMMENT ON TABLE geo.counties IS 'Legacy table that stores all counties within the United States.'
    ' This table is not used by the new system, but is retained in case it is required in the future.'
...
(
    ...
    CONSTRAINT ck_land_use_name CHECK (length(land_use_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_ecosystem_name CHECK (length(ecosystem_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_habitat_name CHECK (length(habitat_name) > 0),
...
(
    ...
    CONSTRAINT ck_unit_name CHECK (length(unit_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0)
);
...
(
    ...
    CONSTRAINT ck_system_name CHECK (length(system_name) > 0),
...
(
    ...
    CONSTRAINT ck_waterbody_type_name CHECK (length(waterbody_type_name) > 0)
);
...
(
    ...
    catchment         Geography(MultiPolygon),
    waterbody_type_id SMALLINT,
    waterbody_code    VARCHAR(100),
    waterbody_name    VARCHAR(255),
    metadata          JSON,
    Geography_changed TIMESTAMPTZ        NOT NULL DEFAULT now(),
    created_date      TIMESTAMPTZ        NOT NULL DEFAULT now(),
    updated_date      TIMESTAMPTZ        NOT NULL DEFAULT now(),

    CONSTRAINT ck_site_name CHECK (length(site_name) > 0),
    CONSTRAINT ck_waterbody_code CHECK (length(waterbody_code) > 0),
    CONSTRAINT ck_waterbody_name CHECK (length(waterbody_name) > 0),
...
/*
 ...
 this site.
 */
CREATE OR REPLACE FUNCTION fn_before_update_geography()
    RETURNS trigger AS
$$
BEGIN
    NEW.geography_changed = now();
    RETURN NEW;
END ;
$$
    LANGUAGE 'plpgsql';
 ...
 */
CREATE TRIGGER tr_sites_geography_update
    BEFORE UPDATE
        OF catchment, location
    ON geo.sites
    FOR EACH ROW
EXECUTE PROCEDURE fn_before_update_geography();
...
(
    ...
    catchment    Geography(MultiPolygon) NOT NULL,
    description  TEXT,
    created_date TIMESTAMPTZ             NOT NULL DEFAULT now(),
...
(
    ...
    CONSTRAINT ck_predictor_type_name CHECK (length(predictor_type_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_predictor_name CHECK (length(predictor_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0),
...
(
    ...
    CONSTRAINT ck_model_type_name CHECK (length(model_type_name) > 0),
    CONSTRAINT ck_model_abbreviation CHECK (length(abbreviation) > 0)
);
...
(
    ...
    CONSTRAINT ck_organization_name CHECK (length(organization_name) > 0),
...
(
    ...
    CONSTRAINT ck_first_name CHECK (length(first_name) > 0),
    CONSTRAINT ck_last_name CHECK (length(last_name) > 0),
...
(
    ...
    CONSTRAINT ck_role_name CHECK (length(role_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_level_name CHECK (length(level_name) > 0),
...
(
    ...
    CONSTRAINT ck_life_stage_abbreviation CHECK (length(abbreviation) > 0),
    CONSTRAINT ck_life_stage_name CHECK (length(life_stage_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_synonym CHECK (length(synonym) > 0),
...
CREATE TRIGGER tr_synonym_update
    BEFORE UPDATE
    ON taxa.synonyms
    FOR EACH ROW
EXECUTE PROCEDURE fn_before_update();
...
(
    ...
    CONSTRAINT ck_attribute_name CHECK (length(attribute_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_source_name CHECK (length(source_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0)
);
...
(
    ...
    CONSTRAINT ck_translation_name CHECK (length(translation_name) > 0)
);
...
(
    ...
    CONSTRAINT fk_translation_taxa_translation_id FOREIGN KEY (translation_id) REFERENCES taxa.translations (translation_id) ON DELETE CASCADE,
...
comment on column taxa.translation_taxa.is_final is 'True when a taxa is part of the core model. False means
    ...
    used as part of the final model calculation.';
...
(
    ...
    CONSTRAINT ck_authors CHECK (length(authors) > 0)
);
...
(
    ...
    CONSTRAINT ck_model_name CHECK (length(model_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0),
...
CREATE TABLE geo.model_conditions
(
    condition_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    model_id     SMALLINT    NOT NULL,
    display_text VARCHAR(50) NOT NULL,
    description  TEXT,
    condition    numrange    NOT NULL,

    CONSTRAINT ck_display_text CHECK (length(display_text) > 0),
    CONSTRAINT fk_model_conditions_model_id FOREIGN KEY (model_id) REFERENCES geo.models (model_id) ON DELETE CASCADE
);
CREATE UNIQUE INDEX ux_model_conditions_model_id ON geo.model_conditions (model_id, display_text);
CREATE UNIQUE INDEX ux_model_conditions_model_condition ON geo.model_conditions (model_id, condition);
CREATE INDEX ix_model_conditions_condition ON geo.model_conditions USING gist (condition);
...
(
    ...
    -- Do not cascade. You cannot delete a predictor if it is used by a model
...
-- TODO: should created_date really by part of primary key? Are multiple predictor values allowed per site?
...
(
    ...
    CONSTRAINT ck_sample_method_name CHECK (length(sample_method_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_sample_type_name CHECK (length(sample_type_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_box_states CHECK (box_state_order > 0),
    CONSTRAINT ck_box_state_name CHECK (length(box_state_name) > 0)
);
...
(
    ...
    field_notes         TEXT,
    lab_split           REAL        NOT NULL,           -- lab puts all sample in bucket and sorts 0.25 (25%) or 0.5 (50%)
...
(
    ...
    CONSTRAINT ck_mass_method_name CHECK (length(mass_method_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_mass_type_name CHECK (length(mass_type_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0)
);
...
(
    ...
    CONSTRAINT ck_note_name CHECK (length(note_name) > 0),
    CONSTRAINT ck_abbreviation CHECK (length(abbreviation) > 0)
);
...
(
    ...
    CONSTRAINT ck_asset_name CHECK (length(asset_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_predictor_value CHECK (length(predictor_value) > 0)
);
...
CREATE TABLE metric.metric_groups
(
    group_id     SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT ck_group_name CHECK (length(group_name) > 0),
    CONSTRAINT ck_metric_groups CHECK (sort_order > 0)
);
...
(
    ...
    CONSTRAINT ck_formula_name CHECK (length(formula_name) > 0),
    CONSTRAINT ck_code_function CHECK (length(code_function) > 0)
);
...
(
    metric_id         SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT ck_metric_name CHECK (length(metric_name) > 0),
    CONSTRAINT fk_metrics_metric_type_id FOREIGN KEY (group_id) REFERENCES metric.metric_groups (group_id),
    CONSTRAINT fk_metrics_formula_id FOREIGN KEY (formula_id) REFERENCES metric.formulae (formula_id)
);
CREATE INDEX fx_metrics_metric_type_id ON metric.metrics (group_id);
CREATE INDEX fx_metrics_formula_id ON metric.metrics (formula_id);
...
(
    ...
    CONSTRAINT fk_metric_metric_taxa_taxonomy_id FOREIGN KEY (taxonomy_id) REFERENCES taxa.taxonomy (taxonomy_id) ON DELETE CASCADE
);

CREATE TABLE metric.metric_attributes
(
    metric_id    SMALLINT NOT NULL,
...
CREATE TABLE metric.report_types
(
    report_type_id   SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    report_type_name VARCHAR(255) UNIQUE NOT NULL,
    display_text     TEXT, -- markdown text for reporting
    description      TEXT,
    metadata         JSON,
    ...
    CONSTRAINT ck_report_type_name CHECK (length(report_type_name) > 0)
);
CREATE TRIGGER tr_report_types_update
    BEFORE UPDATE
    ON metric.report_types
    FOR EACH ROW
EXECUTE PROCEDURE fn_before_update();
...
(
    analysis_id    SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT ck_analysis_name CHECK (length(analysis_name) > 0),
    -- CONSTRAINT fk_analyses_analysis_type_id FOREIGN KEY (analysis_type_id) REFERENCES metric.analysis_types (analysis_type_id),
    CONSTRAINT fk_analyses_report_id FOREIGN KEY (report_type_id) REFERENCES metric.report_types (report_type_id),
...
CREATE UNIQUE INDEX ux_metric_analyses_report_id ON metric.analyses (report_type_id, sort_order);
...
CREATE TABLE metric.analysis_metrics
(
    analysis_id SMALLINT NOT NULL,
    metric_id   SMALLINT NOT NULL,

    CONSTRAINT pk_metric_analysis_metrics PRIMARY KEY (analysis_id, metric_id),
    CONSTRAINT fk_metric_analysis_metrics_analysis_id FOREIGN KEY (analysis_id) REFERENCES metric.analyses (analysis_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_analysis_metrics_metric_id FOREIGN KEY (metric_id) REFERENCES metric.metrics (metric_id) ON DELETE CASCADE
);

CREATE TABLE metric.rarefactions
(
    rarefaction_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    sample_id      INT         NOT NULL,
    translation_id SMALLINT    NOT NULL,
    fixed_count    SMALLINT    NOT NULL,
    code_version   varchar(50) NOT NULL,
    rarefied_taxa  JSONB,
    created_date   TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    CONSTRAINT fk_metric_runs_sample_id FOREIGN KEY (sample_id) REFERENCES sample.samples (sample_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_rarefactions_translation_id FOREIGN KEY (translation_id) REFERENCES taxa.translations (translation_id)
);
CREATE UNIQUE INDEX ux_metric_runs_sample_id ON metric.rarefactions (sample_id, translation_id, fixed_count, code_version);

CREATE TABLE metric.metric_values
(
    rarefaction_id INT NOT NULL,
    metric_id      INT NOT NULL,
    metric_value   REAL,
    metadata       JSON,

    CONSTRAINT pk_metric_sample_metrics PRIMARY KEY (rarefaction_id, metric_id),
    CONSTRAINT fk_metric_sample_metrics_rarefaction_id FOREIGN KEY (rarefaction_id) REFERENCES metric.rarefactions (rarefaction_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_sample_metrics_metric_id FOREIGN KEY (metric_id) REFERENCES metric.metrics (metric_id)
);
CREATE INDEX fx_metric_sample_metrics_metric_id ON metric.metric_values (metric_id);
...
(
    ...
    CONSTRAINT fk_metric_reports_box_id FOREIGN KEY (box_id) REFERENCES sample.boxes (box_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_reports_project_id FOREIGN KEY (project_id) REFERENCES sample.projects (project_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_reports_customer_id FOREIGN KEY (customer_id) REFERENCES entity.entities (entity_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_reports_model_id FOREIGN KEY (model_id) REFERENCES geo.models (model_id),
    CONSTRAINT ck_metric_reports CHECK ((box_id IS NOT NULL) OR (project_id IS NOT NULL) OR (customer_id IS NOT NULL))
);
CREATE INDEX fx_metric_reports_box_id ON metric.reports (box_id);
CREATE INDEX fx_metric_reports_project_id ON metric.reports (project_id);
CREATE INDEX fx_metric_reports_customer_id ON metric.reports (customer_id);
CREATE INDEX fx_metric_reports_model_id ON metric.reports (model_id);

CREATE TABLE metric.report_samples
(
    report_id      INT NOT NULL,
    rarefaction_id INT NOT NULL,

    CONSTRAINT pk_metric_report_samples PRIMARY KEY (report_id, rarefaction_id),
    CONSTRAINT fk_metric_report_samples_report_id FOREIGN KEY (report_id) REFERENCES metric.reports (report_id) ON DELETE CASCADE,
    CONSTRAINT fk_metric_report_samples_rarefaction_id FOREIGN KEY (rarefaction_id) REFERENCES metric.rarefactions (rarefaction_id)
);
...
(
    ...
    CONSTRAINT ck_line_item_type_name CHECK (length(line_item_type_name) > 0)
);
...
(
    ...
    CONSTRAINT ck_agreement_type_name CHECK (length(agreement_type_name) > 0)
);
