...
CREATE TABLE labs (
    lab_id              INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE agencies (
    ...
    CONSTRAINT fk_agencies_category_id FOREIGN KEY (category_id) REFERENCES agency_categories(category_id)
);
COMMENT ON TABLE agencies IS 'customer agencies (e.g. BLM)';

CREATE INDEX ix_agencies_category_id ON agencies(category_id);
CREATE INDEX ux_agencies_name ON agencies(agency_name, category_id);
...
CREATE TABLE datums (
    datum_id            INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    datum_name          VARCHAR(10) UNIQUE NOT NULL,
    is_visible          BOOLEAN NOT NULL DEFAULT TRUE
);
...
-- TODO: What is the purpose of PilotDB.Customers.Group? Appears long form of agency?
CREATE TABLE customers (
    ...
    CONSTRAINT fk_customers_category_id FOREIGN KEY (category_id) REFERENCES agency_categories(category_id),
    CONSTRAINT fk_customers_agency_id FOREIGN KEY (agency_id) REFERENCES agencies(agency_id)
);
CREATE INDEX ix_customers_country_id ON customers(state_id);
CREATE INDEX ix_customers_category_id ON customers(category_id);
CREATE INDEX ix_customers_agency_id ON customers(agency_id);

CREATE TABLE box_statuses (
    box_status_id           INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE boxes (
    ...
    CONSTRAINT fk_boxes_status_id FOREIGN KEY (box_status_id) REFERENCES box_statuses(box_status_id),
    CONSTRAINT fk_boxes_project_id FOREIGN KEY (project_id) REFERENCES projects(project_id)
);
CREATE INDEX fx_boxes_customer_id ON boxes(customer_id);
CREATE INDEX fx_boxes_status_id ON boxes(box_status_id);
CREATE INDEX fx_boxes_project_id ON boxes(project_id);
...
CREATE TABLE samples (
    ...
    CONSTRAINT fk_samples_lab_id FOREIGN KEY (lab_id) REFERENCES labs(lab_id),
...
CREATE INDEX fx_samples_box_id ON samples(box_id);
CREATE INDEX fx_samples_site_id ON samples(site_id);
CREATE INDEX fx_samples_type_id ON samples(type_id);
CREATE INDEX fx_samples_method_id ON samples(method_id);
CREATE INDEX fx_samples_habitat_id ON samples(habitat_id);
CREATE INDEX fx_samples_lab_id ON samples(lab_id);
...
CREATE TABLE taxa_levels(
    ...
    taxa_level_rank     SMALLINT UNIQUE NOT NULL,
    created_date        TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_date        TIMESTAMPTZ NOT NULL DEFAULT now(),

    CONSTRAINT ck_taxa_levels_taxa_level_rank CHECK (taxa_level_rank > 0)
);
...
CREATE TABLE si_data (
    ...
    CONSTRAINT fk_si_data_sample_id FOREIGN KEY (sample_id) REFERENCES samples(sample_id),
    CONSTRAINT fk_si_data_taxa_id FOREIGN KEY (taxa_id) REFERENCES taxa(taxa_id),
...
-- TODO: confirm that velo means velocitty
CREATE TABLE bug_drift (
    ...
    CONSTRAINT fk_bug_drift_sample_id FOREIGN KEY (sample_id) REFERENCES samples(sample_id),
...
CREATE TABLE organic_matter_types (
    organic_id     INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    organic_name   VARCHAR(20) UNIQUE NOT NULL
);

CREATE TABLE bug_o_matter (
    sample_id           INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    organic_id     INT NOT NULL,
    afdm                FLOAT NOT NULL,
    notes               TEXT,

    CONSTRAINT fk_bug_o_matter FOREIGN KEY (sample_id) REFERENCES samples(sample_id),
    CONSTRAINT fk_bug_o_matter_organic_id FOREIGN KEY (organic_id) REFERENCES organic_matter_types(organic_id),
    CONSTRAINT ck_bug_o_matter CHECK (afdm > 0)
);
CREATE INDEX fx_bug_o_matter_organic_id ON bug_o_matter(organic_id);
...
CREATE TABLE bug_volumes (
    ...
    CONSTRAINT fk_bug_volumes_sample_id FOREIGN KEY (sample_id) REFERENCES samples(sample_id),
